...
const removeExportAndCommas = (content) => (content.replace(/export\s+/g, "").replace(/,\s*/g, "\n"));
const addIndentationToEnums = (enumContent) => {
...
const removeLinesBelowSeparator = async (filePath, separator) => {
    try {
...
const updatePrismaSchema = async () => {
    try {
        ...
        if (dividerIndex === -1) {
            throw new Error(`Enums Divider (${ENUMS_DIVIDER}) not found in schema.prisma.`);
        }
        const beforeDivider = prismaSchemaContent.slice(0, dividerIndex + ENUMS_DIVIDER.length);
        const afterDivider = prismaSchemaContent.slice(dividerIndex + ENUMS_DIVIDER.length);
        const updatedSchemaContent = (`${beforeDivider}\n${cleanedEnumsContent}\n${afterDivider.trim()}`);
